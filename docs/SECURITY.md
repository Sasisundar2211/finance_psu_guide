...
- **Printing enabled, download still blocked (client-approved post-freeze change, DECISIONS.md D15, superseding the print-blocking design this section previously described):** the reader does not intercept or block the browser/OS print action — no `beforeprint` handler, no canvas-blanking before a print dialog can render content. The viewer still exposes no native download UI and no direct file link (item above). **Accepted limitation:** because printing is enabled, the browser/OS print dialog's own "Save as PDF" destination cannot be reliably prevented — this is a known, client-accepted residual risk, not something this build claims to defend against.
...
- **Known residual risk, explicitly out of scope per REQUIREMENTS.md §5:** the above deters casual downloading and copy-paste, but does not prevent a determined user from screenshotting or using browser devtools to intercept the rendered content — and, since printing is intentionally enabled (D15), it neither attempts nor is intended to prevent the browser/OS print dialog's own "Save as PDF" output either. The invoice's own wording — "no normal download button" — and the approved stack doc frame the no-download/text-copy deterrence as the intended bar, not full DRM. Forensic watermarking (present in a *rejected* stack draft) is not in scope unless the client asks for it.
...
- [ ] PDF viewer: no Download control present in the viewer UI; print control present and usable, with print action NOT blocked (client-approved, D15); text-layer copy confirmed non-functional; a chapter read for over 60 seconds confirmed unaffected by presigned-URL expiry (D11.4).
