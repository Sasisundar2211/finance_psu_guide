...
- Standard fields only: `id`, `email`, `password` (hashed), `date_joined`, plus Django's native `is_staff`/`is_superuser` and the standard permissions/groups system.
...
- `id`, `user` (FK → User, one-to-one), `active_session_key` (tracking hash — the current session's key; a mismatch on any request triggers `session.flush()` + `logout()` + redirect to `/login/?error=session_conflict`, per REQ-ACC-03), `phone_number` (CharField, max_length=15, null=True).
