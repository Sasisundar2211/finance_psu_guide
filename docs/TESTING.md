...
- `UserProfile.active_session_key` invalidation logic on new login: second login overwrites the key; the first session's next request hits `session.flush()` + `logout()` + redirect to `/accounts/login/?error=session_conflict` (route corrected, DECISIONS.md D17).
...
7. **Second Google login on another device invalidates the first session:** a student logs in via Google on device A, then via Google again on device B → device A's next request lands on `/accounts/login/?error=session_conflict` (route corrected, DECISIONS.md D17), identical to REQ-ACC-02/03's existing behavior for local logins.
...
- [ ] Second login on another browser/device: first session's next request lands on `/accounts/login/?error=session_conflict` (route corrected, DECISIONS.md D17), not a silent failure.
