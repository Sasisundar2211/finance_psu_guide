...
- **REQ-COURSE-03** [Confirmed — mixed provenance, corrected this pass, DECISIONS.md D11.6] No PDF download, no print, no text copy: no download button; normal browser download/save is not offered by the reader UI; printing is blocked; the canvas-rendered text layer's copy/select is unbound.
  *Provenance split:* the invoice/workflow doc mandate only "no normal download button" / online-only viewing. The approved technical stack additionally describes stripping print and text-copy paths. The current implementation baseline retains all three as the build's scope — that's a real, current requirement of this build, but not all three trace to the invoice itself; don't read this line as "the invoice mandated print/copy blocking."
  *Acceptance:* No download control is present in the viewer; the PDF is not linked as a direct downloadable file; the browser print shortcut/menu is intercepted and blocked before a print dialog can render the content; selecting/copying text out of the viewer does not work (PDF.js text layer disabled, or its `user-select` and `copy` event explicitly unbound). *(Note: this is deterrence, not DRM — see SECURITY.md §5 for residual risk and what is explicitly out of scope.)*
...
- Forensic/dynamic watermarking of PDFs — present in one *rejected* stack draft, not in the approved stack spec or the invoice. **Provenance correction (DECISIONS.md D11.6):** PDF protection scope isn't all invoice-mandated — the invoice/workflow doc specify "no normal download button" / online-only viewing; the approved technical stack additionally describes stripping print and text-copy paths; the current implementation baseline (REQ-COURSE-03) retains all three (download/print/copy deterrence) as the build's chosen scope, not because the invoice itself mandated print/copy blocking.
